...
class HybridCompatibleNitroView : HybridCompatibleNitroViewSpec {
  ...
  private func startTimer() {
    ...
    timer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] _ in
      ...
      self.counter += 1
      print("Swift Timer from new Arch: \(self.counter) callback: \(String(describing: self.onTick))")
...
@objc(LegacyCompatibleNitroView)
public class LegacyCompatibleNitroView : NSView {
    lazy var view: NSView = {
      let v = NSView()
      v.wantsLayer = true
      startTimer()
