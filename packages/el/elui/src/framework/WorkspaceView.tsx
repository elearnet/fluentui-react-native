      ...
      <View style={[styles.tabListContainer, isLeftCollapsed ? { marginLeft: minSidePaneWidth } : {}]}>
        ...
        >
            {split.children.map((child: any, index: number) => {
                ...
                const icon = child.viewState?.state?.icon; // Optional icon from state
                ...
                console.log(` ${title}: key=${tabKey}, selectedKey=${selectedKey}, isSelected=${isSelected}, showCorners=${isSelected}`);
                return (
                    <Tab
                        key={tabKey}
                        tabKey={tabKey}
                        showSeparator={showSeparator}
                        showInvertedCorners={isSelected}
                        cornerColor={detailBgColor}
                        style={[styles.tab, {
                            backgroundColor: isSelected ? detailBgColor : 'transparent',
                            //width: tabWidth
                        }]}
                    >
                        <HoverableTabContent
