...
import type { RenderType, RenderResult, DirectComponent, LegacyDirectComponent, FinalRender } from './render.types';
...
export function renderForJsxRuntime<TProps>(
  ...
  if (legacyDirect) {
    ...
    return (legacyDirect as unknown as FinalRender<TProps>)(newProps as any, ...React.Children.toArray(children)) as RenderResult;
  }
...
export function renderForClassicRuntime<TProps>(type: RenderType, props: TProps, ...children: React.ReactNode[]): RenderResult {
  const legacyDirect = asLegacyDirectComponent(type);
  if (legacyDirect) {
    return (legacyDirect as unknown as FinalRender<TProps>)(props, ...children) as RenderResult;
  }
